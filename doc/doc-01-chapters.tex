...
\todo{Dať niekam MobileNetV2 keď ho už používam? Asi by som mohol tu dať ešte menší nadpis že mobilenetV2. Alebo celý mobilenet presunúť
do real-time obj det pod niečo ako efficient model architectures alebo tak}
...
\todo{viac o tom že real-time potrebujú byť rýchle a tak}
...
\todo{Nedať to niekde inde? Kam? Alebo treba to vôbec? Asi by stačil footnote niekde v implementácii kde spomeniem že som
trénoval pre-trained modely}
...
quantization will be used \todo{due to limited resources?}.
...
CARLA\todo{footnote} virtual environment. By replacing BottleneckCSP in the
...
\begin{itemize}
    ...
    nevysvetlil transfer learning skôr} which significantly speeds up the
    process.
\end{itemize}
...
\todo{Niekde popísať rozdiel medzi trénvacím, validačným a testovacím datasetom a na čo treba všetky?}
...
as YOLOv8 MobileNetV2). \todo{použité indices alebo celkovo viac o tom. Môžem ale o tom písať v experimentoch, keďže
...
For smaller models, several input resolutions were tested to provide insights into
...
the number of pixels in the input image \todo{áno?}, we decided to test it.
...
\todo{4 examples v gride že ako vyzerajú augmentované snímky?}
...
\begin{tabular}{|l|c|c|c|c|}
    ...
    % TODO raspberry pi instead of ARM cortex?
...
\todo{Toto dať k results ako important things to note alebo tak?}
...
thread.
\todo{dať tu výsledky z RPI? Otestovať aj väčší batch?}
...
% Tu dať len pár, asi 4 krivky. Medium, MobileNetV2, Nano 512x288 lebo také
% rozlíšenie má aj mobilenet a nakoniec najmenšie Femto. Kvantizované tu netreba

\todo{Zvyšné PR krivky do príloh. Aj kvantizované?}

\todo{niekde PR krivky vysvetliť. Asi ideálne pred mAP v teórii, no}
...
all models can be seen in \todo{autoref appendix + upraviť vetu ak som to urobil
inak...}.
...
\todo{napísať že sme urobili skript ktorý to počíta? Nech si nemyslia že to je
100\% správne. Kam to ale napísať? Do results sa to asi nehodí, tak skôr do
experimentov. Napísať aj že som IoU thresholdy priemeroval}
...
\todo{nechcem testovať ort na jetsonoch takže by bolo fajn keby pred týmto
experimentom bolo porovnanie ort vs trt na jetsonoch. Ak sa nedá, tak tu napísať
že because trt is expected to perform generally better, ort was not tested...}
...
% že femto a pico sú tak malé modely že tie FPS asi netreba brať príliš
% vážne? Do caption: Please note that the inference speeds for tiny models, such
% as the YOLOv8-pico and YOLOv8-femto, might not be accurate due to them being so
% small.
...
\todo{napísať že je v poriadku že sme skončili pri 32 lebo z grafu vidno že už
ten rozdiel medzi 16 a 32 nie je taký veľký ako medzi 8 a 16. A teda batch 64
by pravdepodobne ani na AGX s najmenším femtom už príliš nepomohol}
...
scales are provided in \todo{autoref appendix mAPvsFPS linear - asi 4 grafy}.
...
\todo{autoref appendix - tabuľka mAP a FPS pre každý model (aj kvantizovaný) na
každom zariadení} which presents the complete mAP and FPS values. Additionally,
...
\todo{Všade kde je to vhodné, zmeniť Raspberry PI na Raspberry PI 4B alebo tak}

\todo{Namely znamená že tam vypíšem všetky! Skontrolovať to všade a opraviť}

\todo{Tak do appendixu pridať viac grafov (asi 4) s lineárnou osou X}

\todo{Do appendixu dať tiež dlhú tabuľku - mAP pre každý model, aj kvantizovaný}

\todo{mAP je mean Average Precision - mean je s malým!}
...
metrics as explained in \todo{autoref kde vysvetľujem mAP}\,--\,mAP with the IoU
...
\begin{table}
    ...
    are explained in \autoref{EvaluationMetrics}. \todo{Don't reference a subsection!}}
    \label{mAPTableSmall}
\end{table}
...
For a complete table of inference speeds of all models (including all
quantized models) with a batch size of 1, please see \todo{do appendixu tabuľku
že model, resolution, kvantizácia a potom FPS hodnoty s batch size 1 podľa
zariadenia (device bude column)? aj mAP? To by som potom nemusel robiť tabuľku
pre 'FPSvsmAPComparison'}.
...
%         and that for comparison, YOLOv8-nano model with $512 \times 288$ input
%         resolution and YOLOv8-femto model with $352 \times 192$ input resolution
%         were selected. A batch size of 8 was selected for tests featuring larger
...
%         the TensorRT inference backend was utilized. TODO doplniť že modely sú FP16}
