...
% malú medzierku medzi číslo a jednotku (\,)
% tildy pred cite a také
% Správne písať et al. v textit
% opraviť spojovníky a pomlčky...
% x-axis má byť $x$-axis
% mAP dávať v % a písať s '%'? Áno!
% uvodzovky: ``slovo''
% je v poriadku že píšem 1000 ale 10 000? Ak nie, opraviť!
% vlna, + to isté pre 'an'
% footnote pre prvý learning rate - nie
% autoref odkazy s veľkými písmenami! chapter, section, subsection, figure, table
% strana 50 graf - použiť normálne farby ako všade inde a čísla dať vedľa barov
% menej autorského plurálu! Hlavne v teórii a tak
% vo fitthesis.cls nájsť riadok s tocdepth a zmeniť na 1
% Ak bude treba upraviť ToC ešte (margins a tak), tak v doc.tex
% neodkazovať na subsection - buď kapitola alebo section
% rámčeky okolo obrázkov vymazať?
% niekde napísať ktoré epochy som vybral
% obsah pamäťového média? Nie
% častejšie písať že som reanotoval DETRAC

% todo Rozšírený abstrakt. Netreba ak nebude čas

% TODO viac related work, ak sa dá. Pozrieť poznámku od Špaňhela
...
% Len naozajstné info, nie hlúposti. A musí byť do jednej strany, pokojne aj pol strany
% Má tomu rozumieť aj neodborník v oblasti!

% Že vehicle detection je dôležitý, bolo by fajn keby bežal na slabých
% zariadeniach aby sme mohli mať veľa kamier a netrebalo ďalšie mochovce.

% Chceme teda vyskúšať najnovšie real-time YOLOv8 detektory a porovnať ich na
% rôznych zariadeniach. Skúšam aj Mobilenetv2 backbone. Aj na troch jetsonoch.

% Spomenúť aj experimenty ktoré budem robiť. Spomenúť aj OpenMMLabs

% Nakoniec krátko vysvetliť štruktúru - v ktorej kapitole čo


% Vehicle detection systems are an~important tool for improving road safety, and
% with the rise of convolutional neural networks and the recent advancements in
% object detection, these systems have become more affordable and accurate. However,
% accurate object detectors still require significant computational resources and
% sacrifices must be made to perform this task in real-time on embedded devices or
...
% In this paper, state-of-the-art real-time YOLOv8 object detectors of different sizes are
% benchmarked on six different devices, including three NVIDIA Jetson embedded
% platforms.
% =====
% A popular group of real-time object detectors is the YOLO (You Only Look Once)
% family, of which the latest, state-of-the-art YOLOv8 model architecture will be
% evaluated in this paper. Several models of different sizes were trained to be
% benchmarked on six different devices, including three NVIDIA Jetson embedded
% platforms.
% % Additionally, the CSP Darknet backbone of one of the models was replaced by MobileNetV2 to 
%
% Additionally, one of the models was trained with the MobileNetV2 backbone to
% compare the base YOLOv8 models to this popular architecture.
%
% =====
...
% YOLOv8 models of different sizes, including one trained with the lightweight
% MobileNetV2 backbone, will be benchmarked across six different devices,
% including three NVIDIA Jetson embedded platforms. To accelerate the process of
% training, deploying and testing different models, this paper utilized
...
% In addition to benchmarking the trained models on all devices, several different
% experiments were conducted, including weight quantization and comparison of
% inference speeds using different inference backends, batch sizes or input
% resolutions of the models.
% =====
% In addition to benchmarking the trained models on all devices in terms of their
% inference speeds (FPS) and mAP (mean Average Precision) metrics, we also aim to
% study the effects of weight quantization, usage of different inference backends,
% batch sizes or input resolutions of the models.
% =====
% In addition to benchmarking the trained models on all devices in terms of their
% inference speeds (FPS) and mAP (mean Average Precision) metrics, we also aim to
% explore various factors impacting performance, including weight quantization,
% various input resolutions of the models or different inference backends and
% batch sizes.
% =====
% ! treba tam napríklad 'including', nech je jasné že toho bude viac:
% In addition to benchmarking the trained models on all devices in terms of their
% inference speeds (FPS) and mAP (mean Average Precision) metrics, we also aim to
% study the effects of weight quantization, usage of different inference backends,
% inference batch sizes or input resolutions of the models.
% =====
In addition to benchmarking the trained models on all devices in terms of their
...
% In \autoref{BackgroundAndRelatedWork}, we begin by providing an~overview of the
% essential concepts, tools and libraries relevant to our study. Following this,
% we present a detailed review of the used datasets (\autoref{DatasetsChapter})
% and discuss the object detectors that were evaluated (\autoref{ModelsChapter}),
% including information about the training process. Next, in \autoref{Experiments}
% we delve into the specifics of our experiments, explaining how they were
% conducted and including important things to note before drawing conclusions from
% the results in \autoref{Results}, where we report our findings in detail and
% offer insights gained. Lastly, we suggest potential areas for future work that
% couldn't be researched in this paper.
% =====
% In \autoref{BackgroundAndRelatedWork}, we begin by providing an~overview of the
% essential concepts, tools and libraries relevant to our study. We then proceed
% with a detailed review of the datasets employed in our research in
% \autoref{DatasetsChapter}, followed by a discussion on the evaluated object
% detectors and their training process in \autoref{ModelsChapter}. In
% \autoref{Experiments}, we delve into the specifics of our experiments, outlining
% the methodologies employed, providing an~overview of the devices used for
% testing and discussing the important aspects to consider when interpreting the
% results. These results are then presented in detail in \autoref{Results}, where
% we share our findings and discuss the insights gained from the experiments.
% Finally, we suggest potential areas for future work that couldn't be researched
% in this paper.
% =====
...
% + obrázok nejaký že input image, feature maps, predictions? Nenašiel som
...
% These libraries will be discussed in more detail later in this chapter.
...
% Large and high-quality datasets are very important when training a CNN-based
...
\chapter{Conclusion}
% V podstate tu môžem písať veci ktoré sľubujem v úvode, akurát sa zamerať na
% ciele v zadaní

% ===== MOCKUP:

% we benchmark various YOLOv8 detectors on different devices, including three NVIDIA Jetsons but also RPI.
% we show that accurate object detectors can run in real time on jetson nano,
% while even the larger like YOLOv8-medium can run in real time on xavier nx or agx xavier.
% Additionally, we show that although it is theoretically possible to run the smallest detectors in real time on raspberry pi,
% the accuracy of these detectors is too small and they are only useful when detecting large objects

% We also show that on jetsons, tensorrt is superior and onnxruntime brings no advantage.
% Batch size helps with high FPS but does almost nothing usually
% decreasing model's input resolution goes approximately hand in hand with inference speed

% mobilenet performs better than nano on CPU (pravda že všeobecne? alebo len na RPI?) but doesn't perform much better on GPU? Or just Jetsons?


% ===== V1:

% In this paper, we benchmarked various sizes of the YOLOv8 object detector on
% different devices to show how far go the possibilities of modern,
% state-of-the-art models in the field of vehicle detection. Along with the
% inference speed (FPS), six mean Average Precision (mAP) metrics were measured
% for each model.

% Six different datasets were collected to provide a large amount of data
% necessary to train a robust vehicle detector, featuring a total of \num{1204795}
% object instances in \num{218821} images.

% Tests were conducted on three NVIDIA Jetson embedded devices designed
% specifically for machine learning algorithms. We show that on these devices, the
% use of the TensorRT inference backend results in significantly higher inference
% speeds than when using the popular ONNX Runtime backend. Using the TensorRT
% backend, models were also quantized to \texttt{FP16} and \texttt{INT8}
% precisions and it is shown that quantizing to \texttt{FP16} results in higher
% inference speeds with little to no decrease in accuracy.

% More importantly, we show that accurate vehicle detectors can run in real-time
% on these devices\,--\, the YOLOv8-nano with mAP of \num{0.53} quantized to
% \texttt{FP16} runs at 31.9 FPS, while even the largest trained model,
% YOLOv8-medium with mAP of \num{0.603} also in \texttt{FP16} precision can be
% used for inference on NVIDIA Jetson Xavier NX and NVIDIA Jetson AGX Xavier with
% speeds of \num{47.49} and \num{49.29} respectively. Of course, the inference
% speeds can be increased further by sacrificing a bit of accuracy by decreasing
% the models' input resolutions or using larger batch sizes.

% On the other hand, the Raspberry Pi 4B, not designed for machine learning, can run
% the smallest YOLOv8-femto model with the input resolution of just $352 \times
% 192$ at \num{14.82} FPS or the YOLOv8-pico with input resolution of $384 \times
% 224$ at around 7 FPS on a single CPU core. These models are however tiny and
% very bad at detecting smaller objects, while also not being very good generally,
% so we're far from any accurate object detection on devices like this in
% real-time.  However, even this device could be used to detect large vehicles in
% an~image if a high accuracy is not necessary.
...
% i7-9850H CPU and the NVIDIA MX150 GPU.

% From the collected data, we can draw additional conclusions about other factors
% that influence the accuracy and inference speed of the evaluated models. For
% example, we confirmed that the decrease in model's input shape is approximately
% directly proportional to the increase in inference speed. We also show that
% using a larger batch size rarely results in a more efficient inference and is
% only significant when small models are inferenced on powerful devices.

% Finally, the YOLOv8 with MobileNetV2 backbone performs generally better than
% similar-sized YOLOv8-nano on CPUs, these models are generally comparable on GPUs
% but our tests indicate that this highly depends on the specific device

% All data from our experiments is also displayed in appendices to allow the reader
% to draw conclusions important to them... Alebo čo

% We therefore consider this paper successful in showing how accurate are vehicle
% detectors that are able to run in real-time on various embedded devices,
% including the popular NVIDIA Jetson Nano or the low-performance Raspberry Pi 4B.
...
% state-of-the-art models in the field of vehicle detection. Along with the
% inference speed (FPS), six mean Average Precision (mAP) metrics were measured
% for each model. The models were trained on a large, diverse dataset.

% Three embedded devices from the NVIDIA Jetson family were evaluated in this
% paper. We show that on the entry-level Jetson Nano, reasonably accurate vehicle
% detectors like the YOLOv8-nano can run in real-time, while on higher-performing
% Jetson Xavier NX or Jetson AGX Xavier, even the largest trained model
% YOLOv8-medium can run at almost 50 FPS when quantized to \texttt{FP16}
% precision. We show that on these devices, quantizing models to \texttt{FP16}
% precision results in higher inference speeds with no real decrease in accuracy.

% On the other hand, we show that the Raspberry Pi 4B could indeed run inference
% of the smallest models in real-time, as it achieved almost 15 FPS with the
% smallest YOLOv8-femto model with the lowest input resolution of just $352 \times
% 192$. However, these tiny models perform poorly when detecting small objects and
% should only be used in special cases where the accuracy requirements are not as
% high and the vehicles are large in the input images.

% To compare these embedded devices with processing units commonly found in
% laptops or desktop computers, we also benchmarked the models on the Intel Core
% i7-9850H CPU and the NVIDIA MX150 GPU. Additionally, we study the effects of
% other factors that influence the accuracy and inference speed of the models,
% including the use of different input resolutions or various batch sizes during
% inference.
...
% on achieving real-time performance. Our experiments involved training several
% models of different sizes on a large, diverse dataset of surveillance-type
% images.

% We have shown that the entry-level NVIDIA Jetson Nano can run reasonably
% accurate vehicle detectors like the YOLOv8-nano in real-time. On
% higher-performing devices, namely the Jetson Xavier NX or Jetson AGX Xavier,
% even the largest of the trained models\,--\, YOLOv8-medium, operated at higher
% than real-time speeds when quantized to \texttt{FP16} precision. We found that
% on these devices, quantizing models to \texttt{FP16} precision results in higher
% inference speeds with no real impact on accuracy, while quantizing to
% \texttt{INT8} precision rarely brings an~advantage over smaller models.

% In contrast, the Raspberry Pi 4B can only run real-time inference of the
% smallest models, which perform very poorly, particularly when detecting small
% objects.  Therefore, vehicle detection on this device is only viable in special
% cases where the accuracy requirements are not as high and the vehicles are large
% in the input images.
...
% other factors that influence the accuracy and inference speed of the models,
% including the use of different input resolutions or various batch sizes during
% inference, or whether the ONNX Runtime inference backend has any advantage over
% TensorRT on NVIDIA Jetson devices.
...
% state-of-the-art vehicle detectors on various embedded devices, including the popular
% NVIDIA Jetson Nano and the low-performance Raspberry Pi 4B. Our research
...
% detector models to run on embedded systems.
...
\chapter{Future Work}
\label{FutureWork}
% knowledge distillation, pruning,.. kvantizácia pre CPU; NCNN, ARM NN, TFLite; SSD alebo YOLOX alebo tak
