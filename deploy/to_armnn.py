"""Converts an ONNX model to ARM NN format"""
import os
import argparse
...
def main(onnx_filepath, armnn_filepath):
    ...
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output_filename",   type=str,   default=paths.deploy_armnn_filename,
                        help="output filename. Leave blank to use paths.deploy_armnn_filename")
    args = parser.parse_args()

    onnx_filepath = os.path.join(args.work_dir, args.input_filename)
    armnn_filepath = os.path.join(args.work_dir, args.output_filename)

    assert os.path.exists(onnx_filepath), f"Deployed ONNX model does not exist at {onnx_filepath}"

    main(onnx_filepath, armnn_filepath)