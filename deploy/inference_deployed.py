...
input_video_filepath = os.path.join(paths.proj_path, "vid", "day_hq.mp4")
output_video_filepath = os.path.join(paths.working_dirpath, "inference_day_hq.mp4") # TODO use
...
input_video = mmcv.VideoReader(input_video_filepath)

frame_orig = input_video.read()
frame = mmcv.imresize(frame_orig, (640, 384))
...
img = mmcv.imresize(frame_orig, (640, 384))
# img = frame[0].copy()
...
for i in range(len(bboxes)):
    ...
    cv2.imwrite(os.path.join(os.path.dirname(__file__), "output_image.jpg"), img)
